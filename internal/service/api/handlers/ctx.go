...
func ConversationsQ(r *http.Request) data.Conversations {
	return r.Context().Value(conversationsCtxKey).(data.Conversations).New()
}

func CtxConversationsQ(entry data.Conversations) func(context.Context) context.Context {
	return func(ctx context.Context) context.Context {
		return context.WithValue(ctx, conversationsCtxKey, entry)
	}
}
